...
//TFAR patch
TFAR_fnc_onLRTangentPressed = compile preprocessFileLineNumbers QUOTE(PATHTOF(scripts\fn_onLRTangentPressed_patch.sqf));
TFAR_fnc_onLRTangentReleased = compile preprocessFileLineNumbers QUOTE(PATHTOF(scripts\fn_onLRTangentReleased_patch.sqf;
...
["BWplus_satcomEH", "OnTangent", {
    ...
    if (_this select 2 == 1 && {!isnull _satCom} && {_this select 4}) then {
        ...
        [parsetext format ["<t size='1.2'>" + localize "STR_BWplus_Satcom_connected" + "<br />" + localize "STR_BWplus_Satcom_Signal" + ": %1</t>", [_satCom] call BWplus_communications_fnc_getConnectionQuality], true, 2, 1] call ACE_common_fnc_displayText;
    } else {
